...
def select_task_by_id(_id):
    ...
    try:
        conn.row_factory = sqlite3.Row  # nos permite que sqlite nos trae en un diccionario
        cur = conn.cursor()
        cur.execute(sql)
        task = dict(cur.fetchone())  # fetchone para obtener un solo valor
        return task
    except Error as e:
...
def select_all_tasks():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(sql)
        task_rows = cur.fetchall()  # fetchall para obtener todos los valores
